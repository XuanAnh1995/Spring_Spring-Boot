...
public class Main {
    private static final EntityManagerFactory emf = Persistence.createEntityManagerFactory("com.luuviet.superapp-PU");   // Gửi thông số cấu hình Server, nhà thầu JPA: Hibernate, nhà thầu JDBC cho JPA class lo để tạo kết nối tới CSDL cụ thể là SQL Server
...
//        getAllStudents();   // select * from style OOP, CODE FIRST
    ...
    public static void getAllLecturers() {
        ...
        List<Lecturer> list = em.createQuery("SELECT x FROM  Lecturer x WHERE x.salary = 20000000", Lecturer.class).getResultList();
